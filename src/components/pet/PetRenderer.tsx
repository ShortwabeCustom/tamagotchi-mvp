...
function ThreeRenderer({ action, view, presentation, reducedMotion = false, failure = "none", onMetrics }: Omit<PetRendererProps, "mode">) {
  ...
  return <div className={styles.scene} data-stage={error ? "fallback" : stage} data-failure={error ?? "none"} data-reduced-motion={reducedMotion || prefersReducedMotion}>
    {!error && <div className={styles.canvas} role="img" aria-label={`Miso de cuerpo completo en su rincón, ${action.emotion}`}>
      <SceneBoundary onError={fail}>
        {qaFailure === "import" ? <FailedImport /> : <PetScene action={stableAction} view={view} presentation={presentation} reducedMotion={reducedMotion || prefersReducedMotion} failure={qaFailure} onStage={ready} onFailure={fail} onMetrics={onMetrics} />}
      </SceneBoundary>
    </div>}
