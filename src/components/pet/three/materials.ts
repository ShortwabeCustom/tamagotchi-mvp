...
export function coatMaterial(region: CoatRegion) {
  ...
  material.customProgramCacheKey = () => `miso-coat-v2-${region}`;
  const mask = {
    head: `
      ...
      float muzzle = (1.0 - smoothstep(-0.36, -0.19, p.y + 0.1 * abs(p.x) + edge * 0.023)) * front;
    ...
    leg: `float sock = 1.0-smoothstep(-0.43,-0.25,p.y+edge*0.065); coat=mix(ginger,charcoal,max(sock,smoothstep(0.44,0.61,p.y+p.x*0.21+edge*0.07)));`,
